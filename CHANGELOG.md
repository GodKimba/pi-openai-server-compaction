...
## Unreleased
- add global-only `astraTarget` opt-in for the single exact `cliproxy-main-400k/openai-responses/gpt-6-astra` identity and static base using the existing CLIProxy Responses v2 path; context windows remain in Pi's catalog, with selected-identity auth and no new WebSocket transport
